...
\begin{lemma}
	...
	$r+1$ between any two vertices $u,v\in X$, and (2)
	a.a.s $G^\prime_n$ does not contain a cycle containing
	any vertex $u\in X$.
\end{lemma}
\begin{proof}
	...
	Let $P$ be a path of size $r+1$ between two vertices $w,z$,
	and let $u,v$ be two vertices from $X$. 
	Given $f$ 	
\end{proof}
...
\begin{theorem} 
	...
	\begin{itemize}
		...
		vertices, and let $\overbar{v}\in \N*$ be another list of 
		 Let $r_1,\dots r_k\in \N$ and let
		$\mathcal{T}_1,\dots \mathcal{T}_k$ be 
		$k$-equivalence classes for trees such that
		each $\mathcal{T}_i$ has radii bounded by $r_i$. Let
		$\overbar{v}$ denote a list of fixed vertices 
		$v_1,\dots,v_k,\dots, v_l\in \N$, and let 
		$\phi(\overbar{x})$ be an open sentence on $l$ variables.
		Let $G_n^\prime=G_n\setminus E[\overbar{v}]$ 
	\end{itemize}
