...
\]
Denote this limit by $\mathrm{Pr}[\mathcal{T}]$. Notice that the definition of  $\mathrm{Pr}[\mathcal{T}]$ does not depend by the
choice of $v$.\\

An easy observation is that if we are given a finite
set of vertices $X \subset \N$ and some

 
vertex $v\in \N$ and a $k$-equivalence class
of trees $\mathcal{T}$ with radius at most $r$, 
