...
the parameter $\beta$ imply that FO properties cannot "capture" sudden
...
a sub-hypergraph $H_2\subseteq G$ isomorphic to $H$. A labeled copy
of $H$ in $G$ is a sub-hypergraph $H_2\subseteq G$ together with
an isomorphism $f$ from $H$ to $H_2$. Clearly, the number
...
\begin{itemize}[noitemsep, topsep=0pt]
	...
	\item[(3)] the limit probability of random graph belonging to any of them is an analytic expression on the parameters $\overbar{\beta}$. 
\end{itemize}

After this is archived the theorem follows easily. 

The objective of next sections will be to define the classes $C_1,\dots, C_{n_k}$ and to show that they satisfy properties (1), (2) and (3).
...
$ex(G)>0$.
We want to show that a.a.s any sub-hypergraph 
of $\mathrm{G}^\mathcal{C}(n,\overbar{p})$ whose
diameter is small (i.e. bounded by some constant)
is not dense. \par
It takes a direct computation to show that given
an hypergraph $H$, the number of different (not necessarily
induced) sub-hypergraphs of $\mathrm{G}^\mathcal{C}(n,\overbar{p})$
isomorphic to $H$ belongs to $\Theta(n^{-ex(H)})$.
Indeed, let $N:=|V(H)|$ and let $\phi$ be the automorphism
group of $H$. Then, each sequence $v_1,\dots,v_N$ of $N$
different vertices in $[n]$ can either define a copy of $H$
in $\mathrm{G}^\mathcal{C}(n,\overbar{p})$ or not. Also, each
sequence
...
In this section we want to show that, given various classes of
$k$-equivalence over rooted trees of radii at most $r$,
$\mathcal{T}_1,\dots,\mathcal{T}_l$, and for 
any fixed vertices $v_1,\dots,v_l \in \N$,
the limit probabilities that each $N(v_i;r)$ is a tree which
belongs to $\mathcal{T}_i$ class when rooted at $v_i$ are
independent and they vary analitically with $\overbar{\beta}$.\par
